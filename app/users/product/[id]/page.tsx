    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
                ...
                <p className="mt-6 text-slate-400 font-bold uppercase tracking-widest text-xs animate-pulse">Syncing Secure Node...</p>
            </div>
        );
    }
    ...
    if (!product) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
                ...
                <h1 className="text-2xl font-black text-slate-900 mb-2">Plan Decoupled</h1>
                <p className="text-slate-500 max-w-xs mb-8">The requested investment node could not be retrieved from the mainnet.</p>
                <button
                    onClick={() => router.back()}
            ...
            <main className="pt-28 px-6 max-w-xl mx-auto space-y-10">
                    ...
                    <div className="aspect-[16/10] rounded-[3rem] overflow-hidden bg-slate-100 shadow-2xl shadow-indigo-600/10 border border-slate-200">
                        {product.imageUrl ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-4">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.3em]">Premium Infrastructure</span>
                            </div>
                        )}
                ...
                <div className="bg-white rounded-[2.5rem] p-8 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-slate-100 space-y-8">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Product Price</p>
                            <p className="text-xl font-black text-slate-900 leading-none">{product.price?.toLocaleString()} <span className="text-sm font-bold opacity-40">Br</span></p>
                        </div>
                        <div className="space-y-1.5 border-x border-slate-100 px-4">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Daily Rate</p>
                            <p className="text-xl font-black text-slate-900 leading-none">{product.dailyRate}%</p>
                        </div>
                        <div className="space-y-1.5 text-right">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Daily Income</p>
                            <p className="text-xl font-black text-slate-900 leading-none">{product.dailyIncome?.toLocaleString()} <span className="text-sm font-bold opacity-40">Br</span></p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        <div className="space-y-1.5">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Contract Period</p>
                            <p className="text-xl font-black text-slate-900 leading-none">{product.contractPeriod} <span className="text-sm font-bold opacity-40">Day</span></p>
                        </div>
                        <div className="space-y-1.5 border-x border-slate-100 px-4">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Total Profit</p>
                            <p className="text-xl font-black text-slate-900 leading-none">{product.totalProfit?.toLocaleString()} <span className="text-sm font-bold opacity-40">Br</span></p>
                        </div>
                        <div className="space-y-1.5 text-right">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Principal+Income</p>
                            <p className="text-xl font-black text-slate-900 leading-none">{product.principalIncome?.toLocaleString()} <span className="text-sm font-bold opacity-40">Br</span></p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[2rem] p-8 shadow-[0_15px_45px_-10px_rgba(0,0,0,0.05)] border border-slate-100">
                    <p className="text-slate-600 text-sm font-bold leading-relaxed">
                        Total Withdrawal: Buy Principal {product.price?.toLocaleString()} + Product Income {product.totalProfit?.toLocaleString()} = <span className="text-indigo-600 font-black text-xl">{product.principalIncome?.toLocaleString()} Br.</span>
                    </p>
                </div>
                ...
                <div className="space-y-6 pt-4">
                    ...
                    <div className="text-slate-500 text-[13px] leading-relaxed space-y-4 font-medium">
                        <p>
                            Details: This product is designed as a stable and high-yield investment opportunity, offering reliable daily income throughout the contract period. Investors can withdraw their daily earnings at any time and receive their full principal together with accumulated profit upon completion of the contract cycle.
                        </p>

                        <p>
                            【Product Information】 Price: {product.price?.toLocaleString()} ETB Daily Income: {product.dailyIncome?.toLocaleString()} ETB Contract Period: {product.contractPeriod} Days Daily Rate: {product.dailyRate}% Total Income: {product.totalProfit?.toLocaleString()} ETB Total Withdrawal: Principal: {product.price?.toLocaleString()} ETB Product Income: {product.totalProfit?.toLocaleString()} ETB Total: {product.principalIncome?.toLocaleString()} ETB Purchase Quantity Limit: As per system limit
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-slate-50/50 rounded-3xl p-6 flex items-center justify-between border border-slate-100">
                    <span className="text-sm font-black text-slate-900 uppercase tracking-tight">Purchase quantity limit</span>
                    <div className="px-5 py-2.5 bg-white border border-blue-100 text-blue-600 text-sm font-black rounded-xl shadow-sm">
                        {product.purchaseLimit || 1} times
                    </div>
                </div>
            </main>
            ...
            <div className="p-6 bg-white border-t border-slate-100">
                <div className="max-w-xl mx-auto">
                    ...
                    <p className="text-center text-[9px] font-black text-slate-300 uppercase tracking-widest mt-4">
                        Operation secured by Turner Construction End-to-End Encryption
                    </p>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300">
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl p-8 space-y-8 animate-in slide-in-from-bottom-10 duration-500 overflow-hidden flex flex-col">
                                    ...
                                    {statusMsg.text === "INSUFFICIENT_FUNDS_SPECIAL" ? (
                                        <div className="bg-[#fe395d] rounded-[2rem] p-6 flex flex-col gap-4 shadow-xl shadow-rose-500/10 border border-rose-400/20">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="text-white text-xs font-black uppercase tracking-[0.2em] leading-none mb-1 opacity-80">System Alert</h3>
                                        ...
                                        <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 rounded-[2.5rem] p-8 text-center space-y-6 shadow-2xl shadow-indigo-600/30 relative overflow-hidden">
                                            ...
                                            <div className="relative z-10 flex flex-col items-center">
                                                ...
                                                <h3 className="text-white text-lg font-black uppercase tracking-tight mb-2">Congratulations!</h3>
                                                <p className="text-indigo-50 text-xs font-bold leading-relaxed">
                                                    Now you are a partner of <span className="text-white font-black">Turner Construction Company</span>.
                                                </p>
                                                <div className="mt-6 pt-5 border-t border-white/10 w-full flex flex-col items-center gap-6">
                                                    <p className="text-indigo-200 text-[9px] font-black uppercase tracking-[0.2em]">Invite users and get more rewards!</p>
                                ...
                                <div className="space-y-8 animate-in fade-in duration-500">
                                    ...
                                    <div className="space-y-4">
                                        {[
                                            ...
                                            { label: "PRODUCT PRICE", value: `${product.price?.toLocaleString()} Br`, bold: true },
                                            { label: "DAILY INCOME", value: `${product.dailyIncome?.toLocaleString()} Br`, color: "text-emerald-500" },
                                            { label: "CONTRACT PERIOD", value: `${product.contractPeriod} Day` },
                                            { label: "TOTAL PROFIT", value: `${product.totalProfit?.toLocaleString()} Br`, bold: true },
                                        ].map((item, idx) => (
                                            <div key={idx} className="flex justify-between items-center text-[11px]">
                                                <span className="font-black text-slate-400 uppercase tracking-wider">{item.label}</span>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center text-[11px]">
                                            ...
                                            <span className="font-black text-slate-900">-0.00 Br</span>
                                        </div>

                                        <div className="flex justify-between items-center">
                                            <span className="text-lg font-black text-slate-900">Pay Amount</span>
                                            <span className="text-xl font-black text-indigo-600">{product.price?.toLocaleString()} Br</span>
                                        </div>
                        ...
                        {!statusMsg && (
                            ...
                            >
                                {isBuying ? <Loader2 className="animate-spin" size={24} /> : "CONFIRM"}
                            </button>
                        )}
                    </div>
                </div>
